 ...
 * BSD's ipfw4
 *------------------------------------------------------------------*/
int translate_ipfw4(op, proto, src, log, dst, sports, dports, interface)
int op;
char *proto;
char *src;
char *dst;
...
void include_text_ipfw4(c)
char *c;
{
	if (!strncmp("if(", c, 3)) {
		if (!strncmp("if(ipfw4)", c, strlen("if(ipfw4)"))) {
			matched_if = 1;
			printf("%s", c + strlen("if(ipfw4)"));
		} else
			matched_if = 0;
	} else
		printf("%s", c);
