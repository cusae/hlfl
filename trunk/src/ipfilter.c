...
static char *ipfilter_port(char *port)
{
	char *t;
	if (!port || !strlen(port)) {
		return strdup("");
 ...
 *------------------------------------------------------------------*/
int translate_ipfilter(op, proto, src, log, dst, sports, dports, interface)
int op;
char *proto;
char *src;
int log;
...
char *interface;
{
	...
	case ACCEPT_ONE_WAY:
		fprintf(fout,
			"pass out%s quick %s %s from %s %s to %s %s %s\n",
			logit, via, p, src, sports, dst, dports, icmp_code);
		break;
	case ACCEPT_ONE_WAY_REVERSE:
		fprintf(fout,
			"pass in%s quick %s %s from %s %s to %s %s %s\n",
	...
	case DENY_ALL:
		fprintf(fout,
			"block out%s quick %s %s from %s %s to %s %s %s\n",
			logit, via, p, src, sports, dst, dports, icmp_code);
		fprintf(fout,
			"block in%s quick %s %s from %s %s to %s %s %s\n",
			logit, via, p, dst, dports, src, sports, icmp_code);
		break;
	case REJECT_ALL:
		if (!strcmp(proto, "tcp"))
...
int translate_ipfilter_start(FILE * output_file)
{
	fout = output_file;
